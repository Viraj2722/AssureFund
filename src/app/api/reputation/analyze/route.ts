...
async function fetchGitHubData(octokit: Octokit) {
  try {
    ...
    const totalStarsReceived = repos.reduce(
    ...
    try {
      ...
      try {
        ...
        totalCommits =
          pushEvents.reduce((sum: number, event: any) => {
            return sum + (event.payload?.commits?.length || 0);
          }, 0) * 4; // Estimate for the year

        contributionsLastYear =
          events.filter((e: any) => new Date(e.created_at) > oneYearAgo)
            .length * 4;
      } catch (fallbackError) {
    ...
    const stats = {
      ...
      stars_per_repo:
        repos.length > 0
          ? parseFloat((totalStarsReceived / repos.length).toFixed(2))
          : 0,
      pr_merge_rate:
        totalPRs > 0
          ? parseFloat(((mergedPRs / totalPRs) * 100).toFixed(1))
          : 0,
    };
...
• Pull Requests: ${stats.total_prs} (${stats.merged_prs} merged - ${
        stats.pr_merge_rate
      }% merge rate)
...
Return ONLY valid JSON:
{
  ...
  "reasoning": "<2-3 sentences>"
      ...
      console.log(
        "Continuing without AI enhancement - base score will be used"
      );
  ...
  const reasoning =
    ...
    generateDefaultReasoning(stats, baseScore, finalScore);
...
function generateDefaultStrengths(stats: any, topRepos: any[]): string[] {
  ...
  if (stats.original_repos > 15) {
    strengths.push(
      `Prolific creator with ${stats.original_repos} original repositories`
    );
  }
  ...
  if (stats.account_age_days > 730) {
    strengths.push(
      `Long-term commitment (${(stats.account_age_days / 365).toFixed(
        1
      )} years)`
    );
  }
  if (stats.total_languages > 5) {
    strengths.push(
      `Versatile developer with ${stats.total_languages} languages`
    );
  }
  if (stats.total_stars > 50) {
    strengths.push(
      `Strong community recognition with ${stats.total_stars} stars`
    );
  }
  if (stats.merged_prs > 20) {
    strengths.push(
      `Effective collaborator with ${stats.merged_prs} merged PRs`
    );
  }
  if (stats.pr_merge_rate > 70) {
    strengths.push(
      `High-quality contributions (${stats.pr_merge_rate}% PR merge rate)`
    );
  }
  if (topRepos[0]?.stars > 10) {
    strengths.push(
      `Successful project: "${topRepos[0].name}" with ${topRepos[0].stars} stars`
    );
  }
...
function generateDefaultReasoning(
  stats: any,
  baseScore: number,
  finalScore: number
): string {
  ...
  const quality =
    baseScore >= 60 ? "strong" : baseScore >= 40 ? "moderate" : "developing";
